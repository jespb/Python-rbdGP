...
class Individual:
	...
	def getFitness(self, tr_x = None, tr_y = None):
		...
		if self.fitness is None:
			...
			if self.fitnessType == "Pearson":
				self.fit(self.training_X, self.training_Y)
				self.getTrainingPredictions()
				acc = abs(pearsonr(self.trainingPredictions, self.training_Y)[0])
				if str(acc) == "nan":
					acc = -9999
				#print(acc)
				self.fitness = acc 
	...
	def getTrainingMeasure(self):
		if self.fitnessType in ["Accuracy", "2FOLD", "Pearson"]:
			self.getTrainingPredictions()
			return accuracy_score(self.trainingPredictions, self.training_Y)

		if self.fitnessType == "Pearson":
			self.getTrainingPredictions()
			return pearsonr(self.trainingPredictions, self.training_Y)[1]
	...
	def getTestMeasure(self, test_X, test_Y):
		...
		if self.fitnessType == "Pearson":
			self.getTestPredictions(test_X)
			return pearsonr(self.testPredictions, test_Y)[1]
