...
class Node:
	...
	def create(self, rng, operators=None, terminals=None, depth=None,full=False):
		if depth>1 and (rng.random()<0.5 or full ==True ):
			...
			value = rng.randint(0,len(terminals))
			if value < len(terminals):
				self.value = terminals[value]
			else:
				self.value = rng.randint(0,1000)/1000#rng.random() # Constant in [0,1]
	...
	def calculate(self, sample):
		...
		if self.branches == None:
			...
			if self.value == "log||": # log( |X| )
				res = self.branches[0].calculate(sample)
				res = np.where(res==0, 1, res)
				res = np.where(res<=0, np.log(-res), np.log(res))
				return res
			if self.value == "sin": # sin
				res = self.branches[0].calculate(sample)
				res = np.sin(res)
				return res
			if self.value == "cos": # cos
				res = self.branches[0].calculate(sample)
				res = np.cos(res)
				return res
			if self.value == "tan": # tan
				res = self.branches[0].calculate(sample)
				res = np.tan(res)
				return res
			if self.value == "sig": # sigmoid
				res = self.branches[0].calculate(sample)
				res = 1/(1+np.exp(-res))
				return res
